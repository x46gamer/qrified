...
import { Copy, Download, Loader2, PlusCircle, RefreshCw } from "lucide-react";
import {
  generateQRCode,
  encryptData,
  generateUniqueId,
...
const QRCodeGenerator: React.FC<QRCodeGeneratorProps> = ({ onQRCodesGenerated, lastSequentialNumber }) => {
  ...
  const handleTemplateChange = (value: string) => {
    setTemplate(value as TemplateType);
  };
        ...
        qrPromises.push(
          (async () => {
            ...
            const qrCode: QRCode = {
              ...
              dataUrl,
              template,
              headerText: headerText || (template === 'arabic' ? 'تحقق من المنتج' : 'Verify Product'),
              instructionText: instructionText || (template === 'arabic' ? 'امسح رمز QR للتحقق من صحة المنتج' : 'Scan QR code to verify product authenticity'),
              websiteUrl: websiteUrl || window.location.origin,
              footerText: footerText || (template === 'arabic' ? 'شكراً لاختيارك منتجاتنا' : 'Thank you for choosing our product'),
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid gap-4">
          <div className="space-y-2">
            <Label htmlFor="product-name">Product Identifier (Optional)</Label>
            <Input
              id="product-name"
