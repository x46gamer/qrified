...
import { QrCode, Settings, Users, ChevronRight, ChevronLeft, Globe, LineChart, MessageSquare } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ className }) => {
  ...
  return (
    <div className={cn('flex flex-col h-full bg-white border-r transition-all duration-300', 
      isOpen ? 'w-64' : 'w-[70px]', 
      className
    )}>
      <div className="flex-1 overflow-y-auto py-6">
        <div className="flex items-center justify-between px-3 mb-8">
          ...
          <Button 
            ...
            className={cn("ml-2 rounded-full", !isOpen && "opacity-0")}
          >
        ...
        <nav className="px-3 space-y-1">
          ...
          {isAdmin && (
            <NavLink 
              ...
              className={({isActive}) => cn(
                "flex items-center py-2 px-3 rounded-lg text-sm",
                (isActive || location.search.includes("tab=manage")) 
                  ? "bg-blue-50 text-blue-600" 
                  : "text-gray-700 hover:bg-gray-100"
              )}
            >
