...
    return (key, options = {}) => {
        // A mock translation object, replace with your actual translation logic
        const translations = {
            en: { brandName: 'BrandOS' },
            ar: { brandName: 'براند أو إس' }
        };
...
    const handleNavLinkClick = (e) => {
        // This is a simplified example. In a real app with routing, this would be different.
...
                const headerOffset = headerEl ? (headerEl as HTMLElement).offsetHeight : 70;
...
const CallToActionSection = ({ lang, t }) => {
    // const [isLoading, setIsLoading] = useState(false); // Uncomment if you want loading state

    const handleCheckout = async () => {
        // Optional: Add a loading state to prevent multiple clicks while processing
        // setIsLoading(true);
    
        try {
            // --- IMPORTANT ---
            // Replace this with the actual Price ID from your Stripe Dashboard for this specific "Buy Now" product.
            // This ID starts with 'price_...'.
            const priceId = 'price_XXXXXXXXXXXXXX'; 
    
            if (priceId === 'price_XXXXXXXXXXXXXX') {
                 const errorMessage = "Developer: Please configure a Stripe Price ID inside the handleCheckout function in landingpage.tsx.";
                 console.error(errorMessage);
                 // It's better to show a user-friendly message, but alert is useful for development.
                 alert(errorMessage);
                 return; // Stop the function if the Price ID is not set.
            }
    
            // Use the environment variable for your API/Functions URL.
            // This ensures it works on both localhost and your deployed Vercel site.
            const apiUrl = import.meta.env.VITE_SUPABASE_FUNCTIONS_URL;
            if (!apiUrl) {
                throw new Error("VITE_SUPABASE_FUNCTIONS_URL is not defined. Please check your environment variables.");
            }
            
            // The full path to your serverless function
            ...
            const response = await fetch(endpoint, {
                ...
                headers: {
                    ...
                    // If your function requires authentication (e.g., Supabase JWT), add the Authorization header here
            ...
            if (!sessionId) {
                throw new Error("Failed to retrieve a session ID from the backend.");
            }
            
            // Use the environment variable for your Stripe Publishable Key.
            const stripePublicKey = import.meta.env.VITE_STRIPE_PUBLIC_KEY;
            if (!stripePublicKey) {
                throw new Error("VITE_STRIPE_PUBLIC_KEY is not defined. Please check your environment variables.");
            }
            
            const stripe = await loadStripe(stripePublicKey);
            
            if (!stripe) {
                throw new Error("Stripe.js failed to load. Check your internet connection and ad-blockers.");
            }
            ...
            if (error) {
                // This error is usually displayed to the user by Stripe's UI,
                // but you can log it here for your own debugging.
            ...
            // Display a user-friendly error message, e.g., using a toast notification library
            // For example: toast.error(error.message || 'An unexpected error occurred. Please try again.');
            alert((error as Error).message || 'An unexpected error occurred. Please try again.');
        } finally {
            // Make sure to turn off the loading state
            // setIsLoading(false);
        }
    };
...
            if (e.target instanceof HTMLAnchorElement && e.target.hash && e.target.hash.startsWith('#')) {
...
            }
        };
        document.addEventListener('click', smoothScrollHandler as EventListener);
        return () => document.removeEventListener('click', smoothScrollHandler as EventListener);
    }, []);
