...
  const useCases = [
  ...
  const handleBuyNow = async () => {
    ...
    try {
      setIsLoading(true);
      const sessionId = await createCheckoutSession('prod_STjyRAv61Ylnaf');
  ...
  const bannerItems = [
    {
      ...
      value1: "One-Time Payment",
      value2: "Forever Access",
      icon: DollarSign,
    },
    {
      title: "QR Code Quantity",
      value1: "100,000 Monthly",
      value2: "Unlimited & Scalable",
      icon: QrCode,
    },
    {
      title: "Included Features",
      value1: "All Premium Features",
      value2: "Includes Future Updates",
      icon: Star,
    }
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-800 overflow-x-hidden">
      ...
      <div className="container mx-auto px-4 py-8 md:py-12 relative">
        <div className="text-center max-w-4xl mx-auto animate-fade-in">
          ...
          <p className="text-sm md:text-xl text-blue-100 mb-6 md:mb-8 leading-relaxed px-4 animate-fade-in animation-delay-500">
            This isn't just a deal; it's the foundation of our journey. For the first and only time, secure lifetime access to the ultimate tool for product authenticity and customer connection. When this timer hits zero, this offer is gone forever.<br/><br/>
            <strong className="text-white">Get 100,000 QR codes monthly for life, forever!</strong> And don't worry, if you need more, we will provide more.
          </p>
      ...
      {/* Three-Item Banner Section */}
      <div className="bg-gradient-to-br from-slate-900 via-blue-900 to-slate-800 py-12 md:py-20 relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
            {bannerItems.map((item, index) => (
              <motion.div
                ...
                className="bg-white/10 p-6 rounded-xl shadow-lg border border-blue-500/20 backdrop-blur-sm"
              >
                <item.icon className="h-10 w-10 text-blue-400 mx-auto mb-4" />
                <h3 className="text-xl font-bold text-white mb-2">{item.title}</h3>
                <p className="text-lg text-blue-100">{item.value1}</p>
                <p className="text-md text-blue-200">{item.value2}</p>
              </motion.div>
            ))}
          </div>
        </div>
          ...
          >
            ...
            <ul className="space-y-3 mb-8">
              {[
                ...
                "And if you need more, we will provide more.",
